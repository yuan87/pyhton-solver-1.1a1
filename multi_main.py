...
def runCase(nLoop):
	...
	csv_WriteConf='mast_achor_conf.csv'
	l_write=list()
	l_write.append(ma_conf[0])
	l_write.append(ma_conf[1])
	# join title to list
	l_TitleMastType=['each mast no']+l_mastType[nLoop]
...
if __name__=='__main__':
	...
	lm_anchor0=list(map(list, zip(*lm_anchor00)))
	for r in lm_anchor00:
		print(r)
	## remove all empty in each element in lm_anchor0, lm_anchor is a list of list
	lm_anchor=list(map(lambda aList: list(filter(bool,aList)),lm_anchor0[2:]))

	# read mast type and quantity
	# mast and anchorage data

	ma_conf0=list(map(list,zip(lm[:2])))
	ma_conf=list(map(lambda bList: list(filter(bool,bList)),ma_conf0))
	...
	for nLoop in range(0,noOfStage):
		runCase(nLoop)
